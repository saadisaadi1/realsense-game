...
public class Bow : MonoBehaviour
{
    ...
    private float special_attack_timer;
    public float time_between_specail_arrows;
    private float special_between_timer;
    ...
    private Vector3 special_starting_shooting_position;
    ...
    void Update()
    {
        if (!game_manager.editing && game_manager.in_wave && !using_special_attack)
        {
        ...
        if ((Input.GetKeyUp(KeyCode.Z) || Input.GetKeyUp(KeyCode.Space)) && arrow != null && ready && is_pulling)
        {
            is_pulling = false;
            fired = true;
        ...
        if (has_special_attack && !used_special_attack && game_manager.in_wave )
        {
            if (Input.GetKeyDown(KeyCode.X))
            {
                using_special_attack = true;
                aiming_sign.active = true;
            }
        }

        if (has_special_attack && game_manager.in_wave && using_special_attack)
        {
